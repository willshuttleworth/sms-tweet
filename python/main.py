...
async def sms(Body: str = Form(...), From: str = Form(...)):
    ...
    # verify link: how to confirm that auth requests come from sms channel and not browser?
    #   - map phone to some random value generated by server, server checks again at auth time if this code corresponds to the one it generated earlier
    #   - breaking this: have to guess same value as server
    # TODO: add code to /auth url
    #   add query param to auth function
    #   store code value generated in this function in some global dict
    #       dict maps phone to code
    #   if dict[phone] == code, return page normally
    #   else, return some error page
